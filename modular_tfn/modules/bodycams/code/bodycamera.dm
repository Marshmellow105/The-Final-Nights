/obj/item/bodycamera
	...
	icon = 'modular_tfn/modules/bodycams/icons/bodycamera.dmi'
...
/obj/item/bodycamera/AltClick(mob/user)
	...
	if(do_after(user, 10, src, IGNORE_USER_LOC_CHANGE))
		...
		if(status)
			...
			playsound(user, 'modular_tfn/modules/bodycams/sounds/bodycamera_on.ogg', 23, FALSE)
		else
			icon_state = "bodycamera-off"
			playsound(user, 'modular_tfn/modules/bodycams/sounds/bodycamera_off.ogg', 23, FALSE)
